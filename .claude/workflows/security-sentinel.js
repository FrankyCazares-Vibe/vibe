...
const DIMENSIONS = [
  { key: 'auth', title: 'Authentication & session', focus: 'login / signup / password-reset / @iu.edu / @purdue.edu school-email verification / logout / JWT & cookie handling / account takeover / user + email enumeration. Files: src/app/auth/**, src/app/api/auth/**, src/lib/auth/**, src/lib/supabase/{server,browser,middleware,service,env}.ts, src/proxy.ts (the Next 16 request proxy; there is no root middleware file). Check assumptions about Supabase auth config (email confirmation on/off, password policy) and whether verification tokens are single-use and session-bound.' },
  { key: 'authz', title: 'Authorization, IDOR & data isolation (RLS)', focus: "can user A read or modify user B's data; org owner/admin/member/mod role escalation; service-role routes that skip re-authorization after bypassing RLS; direct-object-reference on thread/message/post/org/channel/event ids. Files: src/app/api/me/**, src/app/api/orgs/**, src/app/api/admin/**, src/app/api/posts/**, src/app/api/users/**. Pull live RLS via the supabase MCP (pg_policies) and confirm the Session-51 policies + column grants actually hold (users, orgs, org_members, org_followers, org_invites, channels, channel_members, messages, posts). Club rules to confirm: only a club's owner or admins can list or remove its followers (src/app/api/orgs/[slug]/followers/**); a hidden club (orgs.hidden_at set) stays invisible to non-members (404 on its page and follow routes, absent from search); org lookups for non-members use the service client, so the route itself must re-check hidden_at and membership; only owners and admins send invites (org_invites); a student can change only the text, tags and draft status of their own posts (posts column UPDATE grants, migration 20260916130000)." },

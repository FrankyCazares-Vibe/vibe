...
**Check the counts before trusting any synthesis.** Every writer stage is handed a truncated slice of the findings: the whole findings list is cut at 60,000, 12,000, 9,000 characters on its way to the writers in `feature-sentinel.js`, and at 9,000 in `security-sentinel.js`, with individual fields trimmed shorter still. Only the feature sentinel's merge stage has a safety net that re-adds anything it dropped; the section writers and summaries have none. On a large run a writer summarizes what it received, says nothing about the rest, and the report still reads as complete. In Session 56 a consolidation agent wrote up 177 of 580 controls this way and it looked finished. Compare the report's finding count with the `result` lines in `journal.jsonl`; if they differ, rebuild the document from the journal.

**Ask the third question: does this break a design principle?** The journal's Principles section (`public/journal/vibe_journal.html`) is the reference: *Personality is professional · Show, don't list · Both sides must win · Warm, not casual · Counts are public, identities are private · Your signal, your control*. Neither sentinel asks this. A fabricated "1.2k followers / 847 connections" sat on the desktop profile for five months and no audit caught it, because nothing was broken and nothing was exploitable; the numbers were simply invented. Read principle 05 carefully: public counts are the product, the offence is fake ones, and the owner-only *who* is what premium sells. It replaced a retired April principle, "No vanity metrics", that did not describe the product. If a principle ever reads wrong against what Franky says today, Franky wins and the text gets rewritten.
